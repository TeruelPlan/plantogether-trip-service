...
import com.plantogether.trip.grpc.*;
...
public class TripGrpcServiceImpl extends TripServiceGrpc.TripServiceImplBase {
  ...
  public void isMember(IsMemberRequest request, StreamObserver<IsMemberResponse> responseObserver) {
    try {
      ...
      responseObserver.onError(
          Status.INVALID_ARGUMENT.withDescription("Invalid UUID format").asException());
    } catch (Exception e) {
  ...
  public void getTripMembers(
      GetTripMembersRequest request, StreamObserver<GetTripMembersResponse> responseObserver) {
    try {
      UUID tripId = UUID.fromString(request.getTripId());
      tripRepository
          ...
          .orElseThrow(
              () ->
                  Status.NOT_FOUND
                      .withDescription("Trip not found: " + tripId)
                      .asRuntimeException());
  ...
  public void getTripCurrency(
      GetTripCurrencyRequest request, StreamObserver<GetTripCurrencyResponse> responseObserver) {
    try {
      UUID tripId = UUID.fromString(request.getTripId());
      Trip trip =
          tripRepository
              ...
              .orElseThrow(
                  () ->
                      Status.NOT_FOUND
                          ...
                          .asRuntimeException());
      responseObserver.onNext(
          GetTripCurrencyResponse.newBuilder()
  ...
  public void getTrip(
      ...
      StreamObserver<com.plantogether.trip.grpc.TripResponse> responseObserver) {
    try {
      UUID tripId = UUID.fromString(request.getTripId());
      Trip trip =
          tripRepository
      ...
      List<TripMember> members = tripMemberRepository.findByTripIdAndDeletedAtIsNull(tripId);
      var response =
          com.plantogether.trip.grpc.TripResponse.newBuilder()
              .setTripId(trip.getId().toString())
              .setName(trip.getTitle())
              .setOrganizerDeviceId(trip.getCreatedBy().toString())
              .addAllMemberDeviceIds(members.stream().map(m -> m.getDeviceId().toString()).toList())
              .build();
      responseObserver.onNext(response);
      responseObserver.onCompleted();
    } catch (IllegalArgumentException e) {
