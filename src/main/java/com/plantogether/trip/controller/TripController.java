...
public class TripController {
  ...
  public ResponseEntity<TripResponse> createTrip(
      Authentication authentication, @Valid @RequestBody CreateTripRequest request) {
    UUID deviceId = UUID.fromString(authentication.getName());
    TripResponse response =
        tripService.createTripWithResponse(
            deviceId, request.getTitle(), request.getDescription(), request.getCurrency());
  ...
  public ResponseEntity<TripPreviewResponse> getTripPreview(
      Authentication authentication, @PathVariable UUID id, @RequestParam UUID token) {
    UUID deviceId = UUID.fromString(authentication.getName());
  ...
  public ResponseEntity<Void> removeMember(
      Authentication authentication, @PathVariable UUID id, @PathVariable UUID memberId) {
