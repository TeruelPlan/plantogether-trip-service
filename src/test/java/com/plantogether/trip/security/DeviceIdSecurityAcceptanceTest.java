 ...
 *
 * <p>AC1: Valid UUID in X-Device-Id → 200 AC2: Missing header → 401 / Invalid UUID → 401 AC6:
 * /actuator/health and /actuator/info are permitAll (no X-Device-Id required)
 */
...
class DeviceIdSecurityAcceptanceTest {
  ...
  void requestWithInvalidUuid_returns401() throws Exception {
    mockMvc
        .perform(get("/api/v1/users/me").header("X-Device-Id", "not-a-uuid"))
  ...
  void requestWithEmptyDeviceId_returns401() throws Exception {
    mockMvc
        .perform(get("/api/v1/users/me").header("X-Device-Id", ""))
        .andExpect(status().isUnauthorized());
  }
  ...
  void requestWithValidDeviceId_isAuthenticated_andReachesController() throws Exception {
    ...
    int status =
        mockMvc
            .perform(get("/api/v1/users/me").header("X-Device-Id", deviceId.toString()))
            .andReturn()
            .getResponse()
            .getStatus();
