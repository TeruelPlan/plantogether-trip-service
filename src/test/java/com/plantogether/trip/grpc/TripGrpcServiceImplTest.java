...
class TripGrpcServiceImplTest {
  ...
  @InjectMocks private TripGrpcServiceImpl grpcService;

  @Mock private StreamObserver<IsMemberResponse> isMemberObserver;

  @Mock private StreamObserver<GetTripMembersResponse> getMembersObserver;

  @Mock private StreamObserver<GetTripCurrencyResponse> getCurrencyObserver;
  ...
  void getTripMembers_returnsMemberList() {
    ...
    TripMember m2 =
        TripMember.builder()
            ...
            .displayName("Bob")
            .build();
    when(tripMemberRepository.findByTripIdAndDeletedAtIsNull(tripId)).thenReturn(List.of(m1, m2));

    grpcService.getTripMembers(
        GetTripMembersRequest.newBuilder().setTripId(tripId.toString()).build(),
        getMembersObserver);

    ArgumentCaptor<GetTripMembersResponse> captor =
        ArgumentCaptor.forClass(GetTripMembersResponse.class);
  ...
  void getTripCurrency_returnsCurrencyCode() {
    ...
    ArgumentCaptor<GetTripCurrencyResponse> captor =
        ArgumentCaptor.forClass(GetTripCurrencyResponse.class);
